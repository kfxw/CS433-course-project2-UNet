...
#         if H != W:
#             if H > W:
#                 image = transforms.functional.pad(image, padding=(0, abs(H-W), 0, 0))
#                 mask = transforms.functional.pad(mask, padding=(0, abs(H-W), 0, 0))
                
#             else:
#                 image = transforms.functional.pad(image, padding=(abs(H - W), 0, 0, 0))
#                 mask = transforms.functional.pad(mask, padding=(abs(H - W), 0, 0, 0))

#         return {'image': image, 'mask': mask}

class PadToSquare(object):

    def __call__(self, sample, canon_size=1376):
        image, mask = sample['image'], sample['mask']
        H, W = image.size
        image = transforms.functional.pad(image, padding=(abs(canon_size - H), abs(canon_size - W), 0, 0))
        mask = transforms.functional.pad(mask, padding=(abs(canon_size - H), abs(canon_size - W), 0, 0))
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        image, mask = sample['image'], sample['mask']
        return {'image': self.apply(image).float(),
                'mask': self.apply(mask).float()}
...
class MicroscopyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return sample['image'], sample['mask'], image_shape if self.with_mask else None
    

class MicroscopyDataset_EVAL(Dataset):
