...
def do_epoch(args, epoch, net, dataset, optimizer, eval=False, K=512*512):
    ...
    for iter, data in enumerate(dataloader):
        ...
        loss = loss_map_flat.topk(K, dim=-1)[0].mean()
...
def run(args):
    ...
    net.loss = torch.nn.BCEWithLogitsLoss(reduction='none')
    ...
    for epoch in range(args.epoch):
        ...
        time2 = time.time()
        logging.info('epoch [{}/{}], time elapse={:.2f}, loss={:.4f}, train_acc={:.4f}'.format(epoch, args.epoch, time2-time1, loss, acc))

        if epoch % args.eval_freq == 0:
            logging.info('Evaluate model...')
            loss, acc = do_epoch(args, epoch, net, val_dataset, None, eval=True)
            logging.info('val_loss={:.4f}, val_acc={:.4f}'.format(loss, acc))
